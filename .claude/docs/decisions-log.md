...
### 92. Architecture multi-thèmes, phase 1 — extraction de style-edito dans `themes/style-edito/`

Contexte : le client a 2 autres gabarits visuels déjà avancés ailleurs (`../style-prestige`, `../style-ludique`, hors de ce repo), à faire cohabiter dans la même appli multi-tenant, sélectionnés par tenant via `Tenants.theme`. Décidé avec le client : les 3 thèmes porteront exactement les mêmes 17 pages que style-edito (les pages "en trop" présentes dans les 2 autres projets sont abandonnées). Cette décision ne construit que la phase 1 (extraction + mécanisme de dispatch) — le portage réel de style-prestige et style-ludique est un chantier à part, pas commencé.

**Ce qui bouge** : `features/`, `shared/components/`, `shared/styles/`, `shared/config/` déplacés (`git mv`) vers `themes/style-edito/{features,components,styles,config}/`. Restent à la racine : `payload.config.ts`, `collections/`, `lib/` (couche de données, 100% partagée quel que soit le thème), `shared/lib/` (résolution tenant/thème, agnostique du rendu). `admin/` ne référençait déjà aucun de ces dossiers déplacés — backend admin confirmé indépendant du rendu public, aucun risque de casse de ce côté.

**Mécanisme de dispatch** : `shared/lib/theme.ts` (nouveau, même forme que `tenant.ts`) — `ThemeName` est une union (`'style-edito'` seul pour l'instant), `getCurrentTheme()` lit `tenant.theme` via `getCurrentTenant()` (déjà `cache()`), et `pickTheme(theme, { 'style-edito': X, ... })` sert de registre explicite par point de dispatch : un `Record<ThemeName, T>` complet est exigé par le compilateur, donc ajouter un 2ᵉ thème à `ThemeName` fera échouer la compilation de tout point de dispatch qui ne lui a pas encore donné son composant — un filet, pas une formalité. Choix fait en amont (discuté avec le client) : un registre explicite plutôt qu'un `import()` dynamique par chaîne, que Webpack ne peut pas analyser statiquement de façon fiable.

Un seul point de dispatch réel construit ce soir : `app/(frontend)/layout.tsx`, qui enveloppe déjà toutes les pages. Son contenu spécifique à style-edito (polices Google, Header/Footer, import CSS global) extrait dans `themes/style-edito/RootLayout.tsx`, respectant un contrat `RootLayoutProps` défini dans `themes/registry.ts` (typé par `Awaited<ReturnType<...>>` sur les 4 fonctions `lib/payload.ts` déjà appelées) ; le layout racine devient un dispatcheur fin qui résout thème + données puis choisit le bon `RootLayout` via `pickTheme()`.

Les 17 routes statiques + la route générique `[...slug]` ne sont **pas** converties en dispatcheurs ce soir — elles continuent d'importer directement les composants de style-edito (chemins mis à jour vers `@themes/style-edito/...`). Choix délibéré : leur convertir en `pickTheme()` aujourd'hui, alors qu'il n'existe qu'une seule entrée possible dans le `Record`, n'apporterait aucune protection ni aucun comportement nouveau — juste du bruit mécanique. Le moment naturel pour le faire est la phase 2 (portage de style-prestige), quand un 2ᵉ composant existera réellement pour chaque route ; le layout racine sert de patron pour ce moment-là.

**Alias** : `@features` retiré (n'a plus de sens hors d'un thème), remplacé par `@themes` (`themes/`) ; nouvel alias `@lib` (`lib/`, root) introduit pour éviter aux fichiers de `themes/style-edito/features/*` de recalculer un chemin relatif profond (`../../../../lib/payload`) à chaque déplacement futur — mis à jour dans `next.config.mjs` (webpack + `sassOptions.includePaths`), `tsconfig.json`, `vitest.config.ts`, cohérents entre eux comme c'était déjà la pratique établie.

**Limite connue, assumée, pas résolue ce soir** : `sassOptions.prependData` (injection auto de `@import "variables.scss"` dans chaque fichier SCSS compilé) reste un mécanisme **global** à toute l'app, repointé sur `themes/style-edito/styles`. Ça fonctionne tant qu'un seul thème existe. Dès qu'un 2ᵉ thème aura son propre `variables.scss` (mêmes noms de variables, valeurs différentes), ce mécanisme global deviendra ambigu — il faudra alors basculer vers des imports relatifs explicites par thème plutôt qu'un `prependData`/`includePaths` partagé. Volontairement pas anticipé maintenant : le résoudre avant qu'un 2ᵉ thème existe réellement pour le tester serait de la sur-ingénierie.
...
### 97. Verrouillage de l'admin sur le tenant du domaine d'accès — incident réel corrigé

Incident réel du 27/08/2026 : connecté à `/admin` via `app.civelo.fr`, une modification de la page "Accueil" est partie sur le tenant Saint-Hilaire-Bonneval (`edito`, id 5) au lieu du tenant "App" (id 6) visé. Cause confirmée en base (`updated_at` sur la mauvaise page) : le sélecteur de tenant du plugin `@payloadcms/plugin-multi-tenant` repose entièrement sur un cookie de session (`payload-tenant`) totalement indépendant du domaine utilisé pour accéder à `/admin` — Next.js ne restreint par ailleurs `/admin` à aucun domaine par défaut. Le plugin n'offre aucune résolution de tenant par domaine côté admin (vérifié dans son code source : uniquement une sélection manuelle via cookie).

Décision produit tranchée avec le client à cette occasion : chaque commune accède à son admin via son propre domaine, verrouillé automatiquement sur son tenant, sans sélecteur — le sélecteur ne doit rester utilisable que depuis un domaine dédié super-admin (`SUPER_ADMIN_DOMAIN`, ex. `admin.civelo.fr`, pas encore mis en place). Une refonte de ce BO super-admin (page "Mes sites" listant les tenants) est explicitement différée par le client à plus tard (utile à partir d'une vingtaine de communes, pas maintenant).

Mécanisme retenu, en 2 temps (`middleware.ts` + `app/(payload)/api/lock-tenant/route.ts`) plutôt qu'une résolution directe dans le middleware : `pg` (donc Payload) a besoin du runtime Node, indisponible pour un middleware sans `experimental.nodeMiddleware` — réservé aux versions canary de Next.js, écarté comme trop risqué pour ce projet en prod (confirmé en testant : `CanaryOnlyError` au démarrage). Le middleware (runtime Edge par défaut) se contente donc de rediriger vers une Route Handler classique (runtime Node) qui fait la vraie résolution par `domaine` et pose 2 cookies : `payload-tenant` (celui du plugin) et `tenant-locked-host` (marqueur maison, évite de refaire l'aller-retour à chaque requête — seul un changement de domaine déclenche une nouvelle résolution). Garde anti-redirection-ouverte sur le paramètre `next` de cette route (accepte uniquement un chemin relatif commençant par un seul `/`).
