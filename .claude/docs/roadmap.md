...
| 2026-09-16 | **Mode brouillon / preview** sur `Pages` : `versions: { drafts: { autosave: false } }`, Draft Mode Next.js posé par `app/(payload)/api/preview` (+ `/disable`), toutes les fonctions de lecture publique de `lib/payload.ts` passent `draft: await isPreviewing()`. Aperçu sur le vrai domaine du tenant, jeton porté à 1h. |
...
- **Garde-fou PDF scanné à l'upload (2026-09-15).** `collections/Documents.ts` accepte aujourd'hui n'importe quel PDF, y compris un scan sans aucune couche de texte (inaccessible). L'`alt` obligatoire sur les images existe déjà (`collections/Media.ts`) ; ce garde-fou est son équivalent côté PDF. Faisable en partie seulement : un hook Payload (ex. `pdf-parse`) peut détecter et bloquer un PDF sans texte extractible du tout — mais ne peut pas garantir une vraie conformité PDF/UA (structure balisée, ordre de lecture), donc ne pas sur-promettre au-delà de ce cas précis. La FAQ du site vitrine a été nuancée en attendant (`civelo-site-produit`, commit `0eb053c`) — à remettre à jour une fois ce garde-fou livré.
...
- **Palette et typographie personnalisées par commune (`Tenants.palette`/`Tenants.typographie`, discuté le 2026-09-17).** Les deux champs existent déjà en base mais ne sont lus nulle part — un seul choix possible (« Par défaut »), purement décoratif pour l'instant. Le mécanisme d'override est en fait déjà à moitié câblé : chaque couleur de base des thèmes est écrite `var(--color-x, <valeur codée en dur>)`, donc injecter `--color-x: <valeur du tenant>` (un `<style>` scopé à `.theme-x`, posé côté `RootLayout`, résolu par tenant comme le reste) suffirait à la faire cascader — les teintes dérivées ($primary-10, $coral-15…) sont calculées en CSS relatif (`oklch(from var(--x) ...)`) donc n'ont pas besoin d'être exposées une par une. **Mais ce n'est pas homogène entre les 4 thèmes** : `moderne` et `classique` ont déjà `var()` sur couleurs ET polices ; `édito` a `var()` sur les couleurs mais pas les polices (codées en dur, ex. `Georgia, serif`) ; `accueillant` a l'inverse (couleurs codées en dur en hex, polices en `var()`). À uniformiser sur les 4 avant un BO générique. Pour les polices, nuance en plus par rapport aux couleurs : une valeur ne suffit pas, il faut aussi charger le fichier (`next/font`/Google Fonts posent aujourd'hui `--font-display`/`--font-body` par THÈME, jamais par tenant) — prévoir un mécanisme de chargement dynamique, pas juste un champ texte libre. Piste d'implémentation : `select` « Par défaut » / « Personnalisée » sur `Tenants.palette`, qui révèle (via `admin.condition`, déjà utilisé ailleurs dans `Pages.ts`) un groupe de color-pickers — un champ par variable `--color-*` du thème actif, noms en dur côté code (pas besoin qu'ils soient eux-mêmes dynamiques).
