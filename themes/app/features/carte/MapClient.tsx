...
// Approximations hex de $primary (oklch 0.36 0.07 195) et $accent (oklch 0.82 0.15 80)
const C_PRIMARY = '#153d4d';
const C_ACCENT  = '#c59b2e';
const C_SENTIER = '#64748b';

const COLORS = {
	hebergement: C_ACCENT,
	'site-visite': C_PRIMARY,
} as const;
...
function createClusterIcon(cluster: any) {
	...
	return L.divIcon({
		...
		html: `<div style="
			...
			color:${C_PRIMARY};box-shadow:0 4px 16px rgba(0,0,0,0.25);
		">${count}</div>`,
		iconSize: [44, 44],
		iconAnchor: [22, 22],
	});
...
function MapResizer({ trigger }: { trigger: number | null }) {
	const map = useMap();
	useEffect(() => { map.invalidateSize({ animate: false }); }, [trigger, map]);
	return null;
}
...
function FlyTo({ id, markersRef }: { id?: string; markersRef: React.RefObject<Map<string, L.Marker>> }) {
	...
	useEffect(() => {
		...
		if (poi) {
			...
			setTimeout(() => { markersRef.current?.get(id)?.openPopup(); }, 1350);
			return;
		}
...
export default function MapClient({ initialId }: Props) {
	...
	useEffect(() => {
		fetch('https://geo.api.gouv.fr/communes/87148?fields=contour')
			.then((r) => r.json())
			.then((data) => {
				if (data.contour) setCommuneGeoJSON({ type: 'Feature', geometry: data.contour, properties: {} });
			})
			.catch(() => null);
	}, []);
	...
	const filtersSection = (
		<div className="carte__sidebar-filters">
			...
			<div className="carte__filter-group">
				{(['hebergement', 'site-visite'] as const).map((cat) => (
					<button
						key={cat}
						className={`carte__filter${filters[cat] ? ' carte__filter--active' : ''}`}
						onClick={() => toggle(cat)}
					>
						<span className="carte__filter-dot" style={{ background: COLORS[cat] }} />
						{LABELS[cat]}
				...
				>
					<span className="carte__filter-dot" style={{ background: C_SENTIER }} />
					Sentiers
				...
				<>
					...
					{visibleSentiers.map((s) => (
						<div key={s.id} className="carte__poi-card">
							...
							<div className="carte__poi-info">
								<span className="carte__poi-badge" style={{ color: C_SENTIER }}>Sentier</span>
		...
		>
			...
			{communeGeoJSON && (
				<GeoJSON
					key="commune"
					data={communeGeoJSON}
					style={{ color: C_PRIMARY, weight: 2.5, opacity: 0.9, fillColor: C_PRIMARY, fillOpacity: 0.05 }}
				/>
			)}

			<MarkerClusterGroup chunkedLoading iconCreateFunction={createClusterIcon} maxClusterRadius={60} showCoverageOnHover={false}>
				{visiblePois.map((poi) => (
					<Marker
						key={poi.id}
						position={[poi.lat, poi.lng]}
						icon={createMarkerIcon(COLORS[poi.category], selectedId === poi.id)}
						eventHandlers={{ click: () => setSelectedId(poi.id) }}
						ref={(instance) => {
						...
						<Popup className="carte__popup-wrap" maxWidth={280} minWidth={280}>
							<div className="carte__popup">
								<div className="carte__popup-photo">
									...
									<span className="carte__popup-badge" style={{ background: COLORS[poi.category] }}>
										{LABELS[poi.category]}
									</span>
								</div>
			...
			{visibleSentiers.map((s) => (
				<Polyline key={s.id} positions={s.coordinates} color={C_SENTIER} weight={3} opacity={0.85} dashArray="8 6">
					<Popup className="carte__popup-wrap" maxWidth={280} minWidth={280}>
						<div className="carte__popup">
							<div className="carte__popup-photo">
								<img src={s.image} alt={s.name} className="carte__popup-img" />
								<span className="carte__popup-badge" style={{ background: C_SENTIER }}>Sentier</span>
							</div>
							<div className="carte__popup-body">
		...
		<div className="carte">
			<aside className="carte__sidebar">
				<div className="carte__sidebar-header">
					<div className="carte__sidebar-eyebrow">Tourisme & découvertes</div>
			...
			<div className="carte__map-wrap">{mapContent}</div>
			...
			>
				...
				<div className="carte__sheet-filters">
					...
					<div className={`carte__sheet-filters-panel${filtersExpanded ? ' carte__sheet-filters-panel--open' : ''}`}>
						<div className="carte__sheet-filters-panel-inner">{filtersSection}</div>
					</div>
				</div>
				{listSection}
			</div>
