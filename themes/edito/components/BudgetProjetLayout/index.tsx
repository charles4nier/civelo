...
import FilterBar from '@themes/edito/components/FilterBar';
...
const statusVariant: Record<ProjetItemData['status'], string> = {
	'À venir': 'sunshine',
	'En cours': 'primary',
	Terminé: 'leaf'
};
...
export default function BudgetProjetLayout({ items }: Props) {
	...
	const [activeKind, setActiveKind] = useState<(typeof secondaryFilters)[number]>('Tous');
	const [filtersOpen, setFiltersOpen] = useState(false);
	const [stuck, setStuck] = useState(false);
	const sentinelRef = useRef<HTMLDivElement>(null);

	const yearFilters = useMemo(() => {
		const set = new Set(items.map((e) => new Date(e.date).getFullYear()));
	...
	}, [activeYear, activeKind, items]);

	const counts = useMemo(() => {
		const map: Record<string, number> = { 'Toutes les années': items.length };
		...
		<>
			...
			<section className={`${CLASS_NAME}__hero`}>
				...
				<div className={`${CLASS_NAME}__hero-content`}>
					...
					<h1 className={`${CLASS_NAME}__title`}>Budget &amp; projets</h1>
					<div className={`${CLASS_NAME}__divider`} />
					<p className={`${CLASS_NAME}__subtitle`}>
						Les finances de la commune et les grands travaux, année par année :<br />
						budgets votés, comptes administratifs et projets municipaux.
					</p>
				</div>
			</section>
			...
			<section className={`${CLASS_NAME}__section`}>
				<div className={`${CLASS_NAME}__header container`}>
					<p className={`${CLASS_NAME}__header-eyebrow`}>Finances &amp; travaux</p>
				...
				<div className={`${CLASS_NAME}__body container`}>
					{filtered.length > 0 ? (
							...
							{filtered
								...
								.map((entry) =>
									entry.kind === 'budget' ? (
										...
										>
											<div className={`${CLASS_NAME}__card-icon ${CLASS_NAME}__card-icon--primary`}>
												<FileText size={18} strokeWidth={1.75} aria-hidden="true" />
											</div>
											<div className={`${CLASS_NAME}__card-body`}>
												<span className={`${CLASS_NAME}__card-type`}>Budget</span>
						...
						<p className={`${CLASS_NAME}__empty`}>Aucun résultat pour cette sélection.</p>
					)}
				</div>
			</section>
		</>
